                ...
                <div v-if='materialList.length > 0'>
                    <table>
                        ...
                        <tr v-for='mat of materialList' :key='mat'>
                            <td>{{ mat.name }}</td>
                            <td class='delete_span' @click='delMat(mat.id)'>удалить</td>
                        </tr>
                    </table>
                </div>
...
export default {
    props: ['allProvider', 'listMaterials', 'instanMaterial', 'getOneMaterial'],
    data() {
        return {
            material: null,
            podMaterial: null,
        ...
        addMaterialToList() {
            ...
            if(this.materialList.length > 0) {
                for(let mat of this.materialList) {
                    if(mat.id == this.podPodMaterial.id)
                        add=false
                }
            }
            if(add) {
                this.materialListId.push(this.podPodMaterial.id)
                this.materialList.push(this.podPodMaterial);
            }
        },
        delMat(id) {
            this.materialList = this.materialList.filter(mat => mat.id != id)
    ...
    async mounted() {
        ...
        if(this.$props.listMaterials) {
             this.materialList = this.$props.listMaterials
             this.$props.materialList.forEach((el) => {
                 this.materialListId.push(el.id)
             })
        }
    }
}
