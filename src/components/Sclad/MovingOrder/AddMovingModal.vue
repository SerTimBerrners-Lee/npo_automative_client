...
export default {
  ...
  methods: {
    ...
    save() {
      if(!this.to_user && !this.to_sklad) return showMessage('', 'Выберите отправителя', 'w');
      if(!this.from_user && !this.from_sklad) return showMessage('', 'Выберите получателя', 'w');
    ...
    toElements(prod, e) {
      ...
      for(const nap of this.new_arr_product) {
        if(nap.id == prod.id) {
