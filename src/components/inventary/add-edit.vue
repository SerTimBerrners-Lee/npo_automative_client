  ...
  methods: {
    ...
    create() {
      if(this.obj.name.length < 1)
        return showMessage('', 'Укажите наименование', 'w', this)
      if(!this.inventaryT)
      ...
      if(this.$route.params.edit == 'edit') {
        ...
        this.fetchUpdateInventary(fd).then(() => {
          setTimeout(() => this.exit(), 3000)
          return showMessage('', 'Успешно обновлено. Перенаправление на главную страницу...', 's', this)
        })
      }

      if(this.$route.params.edit != 'edit') {
        this.createNewInventary(fd).then(() => {
          setTimeout(() => this.exit(), 3000)
          return showMessage('', 'Успешно создан. Перенаправление на главную страницу...', 's', this)
        })
      }
    },
    updateVariable() {
