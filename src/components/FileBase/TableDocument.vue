...
export default {
	...
	methods: {
      ...
      this.keyWhenModalGenerateFileOpen = random(10, 999);
		...
		printOneDoc() {
			if(!this.itemFiles) return showMessage('', 'Выберите документ', 'w');
		...
		printAllDoc() {
			if(!this.white_documents.length) return showMessage('', 'Нет документов', 'w');
			const array_img = [];
			for(const doc of this.white_documents) {
